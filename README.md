...
# prompts.csv format
  ...
  •	or without header, first column still filename, second column prompt:
    img0001.png,"Ultrasound of breast with benign lesion"
    img0002.png,"Ultrasound of breast with suspicious mass"

  Every image used for training must have exactly one row in prompts.csv.
...
Base model: Stable Diffusion v1.5 in diffusers format (can be downloaded from HuggingFace or converted).

LoRA file: a .safetensors file containing the fine-tuned LoRA weights.

During inference:
	•	Load the base SD v1.5 model.
	•	Load the LoRA weights into the UNet (e.g., unet.load_attn_procs(...)).
	•	Use any text prompt (no CSV needed at inference time) to generate 240×240 grayscale-style images in the trained domain.
